/**
 * PROPOSED src/audio/voice.ts - the replacement for src/audio/speak.ts.
 *
 * Kept outside src/ for review; move it to `src/audio/voice.ts` to adopt it.
 * The call sites change only in which identifier they pass, not in shape: every
 * function still returns a promise that settles when the line has finished.
 *
 * Why files instead of speechSynthesis
 * ------------------------------------
 * The old module asked the device to read a bare lowercase character. Which
 * voice answered depended on the iPad, a letter name was sometimes spelled out
 * instead of spoken, and there is no way at all to make it say an isolated /b/
 * without a trailing "uh" - which is the exact habit that stops "b-a-t" from
 * ever becoming "bat". Every line is now generated at build time by
 * `npm run voice` and shipped as a small MP3.
 *
 * What WebKit forces on the design
 * --------------------------------
 *  - Audio is unlocked per document by a real user gesture. The AudioContext in
 *    sfx.ts is already unlocked on first touch; this module reuses that same
 *    context rather than opening a second one, which on iOS would need its own
 *    gesture and would compete for the output.
 *  - Playing through an <audio> element poisons the next SpeechRecognition
 *    session: the recogniser returns nothing at all afterwards. Everything here
 *    goes through decodeAudioData and an AudioBufferSourceNode instead, which
 *    does not.
 *  - The ringer switch silences Web Audio unless navigator.audioSession.type is
 *    "playback". session.ts claims that before the context exists and
 *    re-asserts it after every interruption; nothing extra is needed here.
 *  - decodeAudioData on older WebKit does not return a promise. Both forms are
 *    handled.
 *
 * Adoption note: sfx.ts keeps its AudioContext and master gain in module-local
 * variables. One line has to become an export next to the existing
 * `unlockAudio`:
 *
 *     export function audioNodes(): { ctx: AudioContext; master: GainNode } | null {
 *       const ac = audioContext()
 *       return ac && master ? { ctx: ac, master } : null
 *     }
 *
 * and main.tsx calls `attachAudio(audioNodes)` once at boot. Nothing else in
 * sfx.ts changes, and `setMuted` keeps covering the voice because the voice is
 * routed through the same master gain.
 */
...
export function say(id: string, options: SayOptions = {}): Promise<void> {
  ...
  return enqueue(async () => {
    ...
    if (options.fallback) await synthesize(options.fallback, rate * 0.8)
  })
}
